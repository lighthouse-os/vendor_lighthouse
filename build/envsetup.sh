...
cat <<EOF
Additional LighthouseOS functions:
...
- githubremote:    Add git remote for LighthouseOS Github.
...
function breakfast()
{
    ...
    if [ $# -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            # This is probably just the LIGHTHOUSE model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi

            lunch lighthouse_$target-$variant
        fi
    fi
    return $?
}
...
function lighthouseremote()
{
    ...
    git remote rm lighthouse 2> /dev/null
    local REMOTE=$(git config --get remote.github.projectname)
    local LIGHTHOUSE="true"
    if [ -z "$REMOTE" ]
    then
    ...
    then
        ...
        local PFX="LighthouseOS/"
    else
        local PROJECT=$REMOTE
    fi

    local LIGHTHOUSE_USER=$(git config --get review.review.lighthouse.org.username)
    if [ -z "$LIGHTHOUSE_USER" ]
    then
        git remote add lighthouse ssh://review.lighthouse.org:29418/$PFX$PROJECT
    else
        git remote add lighthouse ssh://$LIGHTHOUSE_USER@review.lighthouse.org:29418/$PFX$PROJECT
    fi
    echo "Remote 'lighthouse' created"
}
...
function githubremote()
{
    ...
    git remote add github https://github.com/LighthouseOS/$PROJECT
...
function makerecipe() {
    ...
    then
        pwd
        lighthouseremote
        git push lighthouse HEAD:refs/heads/'$1'
    fi
    '
}

function lighthousegerrit() {
    ...
    local user=`git config --get review.review.lighthouse.org.username`
...
EOF
            ...
            case $1 in
                ...
                changes|for)
                    if [ "$FUNCNAME" = "lighthousegerrit" ]; then
...
function lighthouserebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        echo "LighthouseOS Gerrit Rebase Usage: "
        echo "      lighthouserebase <path to project> <patch IDs on Gerrit>"
    ...
    git fetch "http://review.lighthouse.org/p/$repo" "refs/changes/$refs" && git cherry-pick FETCH_HEAD
    if [ "$?" != "0" ]; then
...
function cmka() {
    if [ ! -z "$1" ]; then
        for i in "$@"; do
            case $i in
                lighthouse|otapackage|bacon|systemimage)
                    mka installclean
                    mka $i
                    ;;
                *)
...
function repopick() {
    T=$(gettop)
    $T/vendor/lighthouse/build/tools/repopick.py $@
}
function fixup_common_out_dir() {
    common_out_dir=$(get_build_var OUT_DIR)/target/common
    target_device=$(get_build_var TARGET_DEVICE)
    common_target_out=common-${target_device}
    if [ ! -z $LIGHTHOUSE_FIXUP_COMMON_OUT ]; then

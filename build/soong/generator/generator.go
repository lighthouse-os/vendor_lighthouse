...
// Copyright (C) 2018 The lighthouseOS Project
//
...
func init() {
	android.RegisterModuleType("lighthouse_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = lighthouseExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = lighthouseExpandVariables(ctx, dep_file)
	...
	cmd := lighthouseExpandVariables(ctx, String(g.properties.Cmd))
